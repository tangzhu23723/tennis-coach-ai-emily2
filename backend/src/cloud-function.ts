...
// 初始化 CloudBase SDK
const appAuth = tcb.init({
  env: process.env.TCB_ENV_ID || 'tangzhu-d5g3fxav4f47f5237',
});
...
app.get('/api/health', (_req: Request, res: Response) => {
  res.json({ status: 'ok', timestamp: new Date().toISOString(), version: 'cloud-function' });
});

// 视频上传（使用 CloudBase Storage）
app.post('/api/videos/upload', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const originalName = videoFile.originalFilename || 'video.mp4';
    const fileSize = videoFile.size;
    const mimeType = videoFile.mimetype || 'video/mp4';

    // 上传到 CloudBase Storage
    const cloudStorage = appAuth.storage();
    const fileExtension = path.extname(originalName);
    const cloudPath = `videos/${videoId}${fileExtension}`;
    ...
    // 获取临时访问链接
    const downloadUrl = await cloudStorage.getTemporaryUrl(uploadResult.fileID);
    ...
    console.log(`[Video Upload] ${originalName} -> ${downloadUrl}`);
...
export async function main(event: CloudBaseEvent, context: CloudBaseContext) {
  ...
  const timeout = setTimeout(() => {
    console.log('[Cloud Function] Timeout approaching');
  }, 18000); // 18秒，提前预警

  try {
    console.log('[Cloud Function] Request received:', event.path, event.httpMethod);
    ...
    return new Promise((resolve, reject) => {
      ...
      // 处理请求
      app(mockReq, mockRes as Response);
